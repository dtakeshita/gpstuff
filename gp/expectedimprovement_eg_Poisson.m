function EI = expectedimprovement_eg_Poisson(x_new, gp, x, y, fmin, varargin)
% expectedimprovement_eg_Poisson Calculate the negative expected
% improvement for Poisson (or non-Gaussian) variability
%
...
if ~isempty(x)
    ...
    %Ef = Knx*a; Ef=Ef(1:size(x_new,1));%can compute Ef & Varf using gp_pred using Laplace approx.
%     invCKnxt = invC*Knx';
...
% Constrain fullfilment probability - with contraint on parameters,
% currently not modified for Poisson case
if nargin > 6
    constPr = [];
    constPrderEf = [];
    constPrderVarf = [];
...
% gradients
if 0 %nargout>1 %currenlty this part isn't modified for Poisson case
    if size(x_new,1)>1
